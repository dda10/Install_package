...
for entry in config:
    ...
    for profile in http_profiles:
        ...
        if response is not None:
            output = f'url_response,domain={domain},uri={uri},url=https://{domain}{uri} http_response_code={response.status_code},response_time={response_time:.9f},result_type="{result_type}"'
            print(output)
        else:
            print(f'url_response,domain={domain},uri={uri},url=https://{domain}{uri} http_response_code=0,response_time=0.000000000,result_type="Request Timeout"')
